...
paths = ["dataset", "tempvideo", "tempimages"] + [
    f"dataset/{format}" for format in formats
]
...
def ffmpeg_get_crop():
    res = str(
        subprocess.check_output(
            "ffmpeg -ss 15 -i tempvideo/trailer.webm -vframes 300 -vf cropdetect -f null -",
    ...
    return (
        res,
        offset,
    )  # subprocess.call(f"ffmpeg -loglevel quiet -i tempvideo/trailer.webm -vf crop=3840:{res[:4]}:0:0 -c:a copy tempvideo/trailerclean.webm")


def ffmpeg_apply_crop(target, dest, res, offset):
    subprocess.call(
        f"ffmpeg -loglevel quiet -y -i {target} -vf crop=3840:{res}:0:{offset} {dest}"
    )


def ffmpeg_keysplit():
    subprocess.call(
        "ffmpeg -loglevel quiet -y -i tempvideo/trailer.webm -an -f segment -vcodec copy -reset_timestamps 1 -map 0 tempvideo/OUTPUT%d.mp4"
    )


def ffmpeg_split_into_images(target):
    subprocess.call(
        f"ffmpeg -loglevel quiet -y -i {target} tempimages/thumb%04d.jpg -hide_banner"
    )
...
def ffmpeg_copy_to_lower_res(target, res):
    for format in formats[1:]:
        ...
        subprocess.call(
            f"ffmpeg -loglevel quiet -y -i dataset/{formats[0]}/{target} -vf scale={format}:{int(res)/scale} dataset/{format}/{target}"
        )
...
for download_line in url_list:
    ...
    for frame, start, length in grouped(download_line[1:], 3):
        ...
        for format in formats[:]:
            all_present = all_present and (
                os.path.isfile(f"dataset/{format}/{current_image_name}_a.jpg")
            )
            all_present = all_present and (
                os.path.isfile(f"dataset/{format}/{current_image_name}_b.jpg")
            )
    ...
    if need_to_run:
        ...
        for frame, start, length in grouped(download_line[1:], 3):
            ...
            ffmpeg_apply_crop(
                f"tempimages/thumb{start:04}.jpg",
                f"dataset/3840/{current_image_name}_a.jpg",
                crop_res,
                crop_offset,
            )
            ...
            current_image += 1
            ffmpeg_apply_crop(
                f"tempimages/thumb{start+length:04}.jpg",
                f"dataset/3840/{current_image_name}_b.jpg",
                crop_res,
                crop_offset,
            )
        ...
        for format in formats[:]:
            all_present = all_present and (
                os.path.isfile(f"dataset/{format}/{current_image//2}_a.jpg")
            )
            all_present = all_present and (
                os.path.isfile(f"dataset/{format}/{current_image//2}_b.jpg")
            )
        if not all_present:
            exit[0]
    else:
