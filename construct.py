...
def ffmpeg_get_crop():
    res = str(
        subprocess.check_output(
            f"{ffmpeg_command} -ss 15 -i tempvideo/trailer.webm -vframes 300 -vf cropdetect -f null -",
    ...
    return (
        res,
        offset,
    )  # subprocess.call(f"{ffmpeg_command} -loglevel quiet -i tempvideo/trailer.webm -vf crop=3840:{res[:4]}:0:0 -c:a copy tempvideo/trailerclean.webm")


def ffmpeg_apply_crop(target, dest, res, offset):
    subprocess.call(
        f"{ffmpeg_command} -loglevel quiet -y -i {target} -vf crop=3840:{res}:0:{offset} {dest}",
        shell=True,
    )
...
def ffmpeg_keysplit():
    subprocess.call(
        f"{ffmpeg_command} -loglevel quiet -y -i tempvideo/trailer.webm -an -f segment -vcodec copy -reset_timestamps 1 -map 0 tempvideo/OUTPUT%d.mp4",
        shell=True,
    )
...
def ffmpeg_split_into_images(target):
    subprocess.call(
        f"{ffmpeg_command} -loglevel quiet -y -i {target} tempimages/thumb%04d.jpg -hide_banner",
        shell=True,
    )
...
def ffmpeg_copy_to_lower_res(target, res):
    for format in formats[1:]:
        ...
        subprocess.call(
            f"{ffmpeg_command} -loglevel quiet -y -i dataset/{formats[0]}/{target} -vf scale={format}:{int(res)/scale} dataset/{format}/{target}",
            shell=True,
        )
    ...
    for frame, start, length in grouped(download_line[1:], 3):
        ...
        for format in formats[:]:
            all_present = all_present and (
                os.path.isfile(f"dataset/{format}/{current_image}.jpg")
            )
            all_present = all_present and (
                os.path.isfile(f"dataset/{format}/{current_image+1}.jpg")
            )
    ...
    if need_to_run:
        ...
        for frame, start, length in grouped(download_line[1:], 3):
            ...
            ffmpeg_apply_crop(
                ...
                f"dataset/3840/{current_image}.jpg",
                crop_res,
                crop_offset,
            )
        ...
        for format in formats[:]:
            ...
            all_present = all_present and (
                os.path.isfile(f"dataset/{format}/{current_image-2}.jpg")
            )
        if not all_present:
            exit[0]
    else:
